...
function check(){
    ...
    for(i=2;i<=3;i++){
        for(j=2;j<=9;j++){
            ...
            var number1=parseFloat(document.getElementById("numberrow"+index2).innerText);
            var number2=parseFloat(document.getElementById("numbercolumn"+index1).innerText);
            if (Math.abs(answer - Math.pow(number1,number2))<0.0001){
    ...
    document.getElementById("accuracy").textContent=nCorrect.toString() + " / 16 правильных ответов";
}
function reset(){
    var i,j;
    for(i=2;i<=3;i++){
